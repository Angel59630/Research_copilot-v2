...
→ conversations 每轮解析范围名称和处理完成的论文目录
→ rag 将范围元数据动态注入系统提示词
→ Agent 根据自然语言决定使用全部范围或指定论文子集
→ retrieval 将临时编号映射为当前范围内的论文 ID
→ infrastructure 读取 Chroma 并调用 DeepSeek
...
- 论文子集选择是单轮运行状态，不写入会话数据库；临时论文编号不得作为永久标识。
- 分组问题未明确指定论文时默认检索全部处理完成的成员；明确指定但存在歧义时先请求澄清。
...
- `config.py` 使用非空有序的类型化聊天模型注册表，每项包含 provider、服务 URL 和模型名。
- 注册表第一项是默认聊天模型；创建或修改会话时只接受注册表中存在的 provider/name 组合。
- `.env` 仅使用统一 `API_KEY`，既可保存明文，也可使用 `{WINDOWS_ENV_NAME}` 引用 Windows 进程环境变量。
- 花括号引用不从 `.env` 递归解析；Ollama 忽略 API Key，DeepSeek 缺少密钥时返回安全配置错误。
- Embedding 的 Ollama 地址与聊天模型注册项相互独立，允许指向不同服务。
- API Key 不得进入接口响应、前端模型列表、日志或异常详情。
